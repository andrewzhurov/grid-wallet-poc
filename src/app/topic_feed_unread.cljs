(ns app.topic-feed-unread
  (:require [hashgraph.main :as hg]
            ...
            [hashgraph.utils.core :refer [hash= flatten-all] :refer-macros [defn* l letl letl2] :as utils]
...
(def *topic->feed-items-count
  (lazy-derived-atom [at/*topic->subjective-db]
      (fn [topic->subjective-db]
        (l [::derive-*topic->feed-items-count topic->subjective-db])
        (l (->> topic->subjective-db
...
(add-watch *topic->feed-items-count ::update-read-on-new-feed-items
           (fn [_ _ _ topic->feed-items-count]
             (l [::update-read-on-new-feed-items topic->feed-items-count])
...
(add-watch as/*selected-topic ::update-read-on-selected-topic
           (fn [_ _ _ selected-topic]
             (l [::update-read-on-selected-topic selected-topic])
...
(def *topic->feed-items-count-unread
  (lazy-derived-atom [*topic->feed-items-count *topic->feed-items-count-read]
      (fn [topic->feed-items-count topic->feed-items-count-read]
        (l [::derive-unread topic->feed-items-count topic->feed-items-count-read])
...
(add-watch *topic->feed-items-count-unread ::derive-prev->new-unread-count
           (fn [_ _ old-topic->feed-items-count-unread new-topic->feed-items-count-unread]
             (l [::derive-prev->new-unread-count old-topic->feed-items-count-unread new-topic->feed-items-count-unread])
             (->> new-topic->feed-items-count-unread
                  (map (fn [[new-topic new-unread-count]]
                         [new-topic [(get old-topic->feed-items-count-unread new-topic 0) new-unread-count]]))
                  (into {})
                  (reset! *topic->feed-items-count-unread-old+new))))

(defc topic-feed-unread-count-bubble-view < rum/reactive
  [topic]
  (letl2 [[old-count-unread new-count-unread] (rum/react (rum/cursor *topic->feed-items-count-unread-old+new topic))]
